...
static const CGFloat kSpacing = 4 + 6;          //update layoutthumbviews too...
static const CGFloat kDefaultThumbSize = 75 + 105;
...
- (void)layoutThumbViews {
	switch ([UIApplication sharedApplication].statusBarOrientation) {
		...
		case UIInterfaceOrientationLandscapeLeft:
			_thumbOrigin = CGPointMake(kSpacing - 5, 0);
			_thumbSize = 75 + 93;
			break;
	}
    CGRect thumbFrame = CGRectMake(self.thumbOrigin.x, self.thumbOrigin.y,
  ...
  if (_columnCount != columnCount) {
    ...
    for (NSInteger i = _thumbViews.count; i < _columnCount; ++i) {
      TTThumbView* thumbView = [[[TTThumbView alloc] init] autorelease];
        //[thumbView setIsDrawBoundray:YES];
