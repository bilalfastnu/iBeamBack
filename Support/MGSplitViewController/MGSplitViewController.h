...
@property (nonatomic, assign) id <MGSplitViewControllerDelegate> delegate;
...
// Actions
- (void)toggleSplitOrientation:(id)sender; // toggles split axis between vertical (left/right; default) and horizontal (top/bottom).
- (void)toggleMasterBeforeDetail:(id)sender; // toggles position of master view relative to detail view.
- (void)toggleMasterView:(id)sender; // toggles display of the master view in the current orientation.
- (void)showMasterPopover:(id)sender; // shows the master view in a popover spawned from the provided barButtonItem, if it's currently hidden.
